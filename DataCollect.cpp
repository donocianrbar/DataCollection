...
void DataCollect::dataCollectProc(vector<float> data,vector<Scalar> color,const String winname)
{
    ...
    for (int i = 0; i < data.size(); i++) {
        ...
        {
            cout<<"Error: The number of data vectors exceeding the maximum!";
            return;
        }
    }
    data_q_h++;
...
void DataCollect::dataCollect(float data,Scalar color,int data_q_index,const String winname)
{
    ...
    putText(coll, fdata, Point(canvas_w-35+offset_t, canvas_h-data+offset_v), 1,1,color,1,8);
    ...
    for(int j = 0; j < ndataSize - 1; j++)
    {
        ...
        //第3个参数：当前点（当前的数据位置）
        line(coll, Point(interval * (ndataSize - j - 1) + offset_t,
                             canvas_h - data_q[data_q_index][(p_data - 1) % canvas_w] + offset_v),
             Point(interval * (ndataSize - j) + offset_t,
                       canvas_h - data_q[data_q_index][p_data % canvas_w] + offset_v),
             color, 1, 8, 0);
    }
//    data_q_h++;
}
...
void DataCollect::gui()
{
    ...
    createTrackbar("proportion_v*100","gui",&proportion_v_subhalf,200,NULL);
    createTrackbar("proportion_t*100","gui",&proportion_t_subhalf,200,NULL);
    proportion_v=proportion_v_subhalf/100.0;
    proportion_t=proportion_t_subhalf/100.0;
    
    //设置偏移
    createTrackbar("offset_v+h/2","gui",&offset_v_subhalf,canvas_h,NULL);
    createTrackbar("offset_t+w/2","gui",&offset_t_subhalf,canvas_w,NULL);
